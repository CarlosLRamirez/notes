---
id: 20250504T041600-linux-upskill-day-3
aliases:
  - My Linux Upskill Challenge Day 3
tags:
  - linux
created: 2025-05-04 16:00
folder: Linux
modified: 2025-12-10 13:43
publish: true
...
- `sudo` is a command that allows a user to execute commands with elevated privileges — typically as `root`. What a user is allowed to run with `sudo` is defined in the _sudoers_ configuration.
...
- You can use the command `sudo -i` to fully become the `root` user. That means from now on, you won’t need to prefix commands with `sudo` for any high-privilege tasks. This is handy when you have a series of commands to run as “_root_.” You’ll also notice that the prompt changes. **I recommend doing this with caution.**
- To get back to your normal "_sudoer_" account you need to type `exit` or `logout`
...
Probably the current `hostname` of the server is what you see in the prompt after "_username@_", however if you want to double check, there are several ways to do it on linux: you can issue any of this commands: `hostname` , `uname-h`, or check the content of this file: `cat /etc/hostname`.
...
- `sudo -i` launches a root shell—it's like starting a new shell session as if you had logged in directly as the root user. It also loads the root user's environment and places you in the `/root` directory (`cd /root`). If you type `echo $HOME`, it will point to `/root`. The `-i` stands for "_simulate initial login_".
- `sudo -s` also launches a root shell, but it doesn’t load the root environment and keeps you in your current working directory. The `-s` stands for "_shell_", and it's less powerful than `-i`.
