...
_Press Ctrl+D to save and exit_
...
> **Tar** is short for _"tape archive"_, and it comes from the early days of Unix, when you actually saved your data on physical tapes.
...
System _Cron_ also manage other **cronjobs** that run daily, weekly and monthly. Those are scripts that you can put on pre-defined directories:
...
Today's Linux distributions generally include `systemd`. Besides starting and stopping services, we can also use it to run task at specific times via the _"timers"_ feature.

We can see the current _"timers"_ configuration with:
