---
id: 20250508T131400-linux-upskill-day-4
aliases:
...
- Everything on your Linux system is located under the `/` directory, also called the _root_ directory. (Don't confuse it with the `/root` directory.)
...
- Some interesting files under `/etc`:
  ...
  - `/etc/ssh/sshd_config`: this is the configuration file for the SSH daemon (`sshd`), which acts as the _server_ for the SSH connection. (The _client_ is on your computer! 💻)
